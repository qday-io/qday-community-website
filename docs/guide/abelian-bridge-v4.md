...
# Abelian Bridge Ver 4.0 User Guide
...
![MetaMask Wallet Authorization](/abel-bridge-v4/01-authorize-wallet.png)
...
![Enter Low-Amount Bridge Details](/abel-bridge-v4/02-low-amount-details.png)
...
![Bridge Direction and Max Balance Options](/abel-bridge-v4/03-direction-and-max.png)
...
![Large-Amount Transaction Warning](/abel-bridge-v4/04-large-amount-warning.png)
...
![Confirmation Pop-up for ABEL to WABEL](/abel-bridge-v4/05-confirm-abel-to-wabel.png)

*Confirmation pop-up for ABEL -> WABEL bridge.*

![Confirmation Pop-up for WABEL to ABEL](/abel-bridge-v4/06-confirm-wabel-to-abel.png)
...
![Scan QR Code for ABEL to WABEL Deposit](/abel-bridge-v4/07-scan-qr-code.png)
...
![Bridge Successful for ABEL to WABEL](/abel-bridge-v4/08-bridge-success-abel-to-wabel.png)
...
![Transfer WABEL Deposit Screen](/abel-bridge-v4/09-transfer-wabel.png)
...
![Bridge Successful for WABEL to ABEL](/abel-bridge-v4/10-bridge-success-wabel-to-abel.png)
...
![Large-Value Transaction Failed Screen](/abel-bridge-v4/11-large-transaction-failed.png)
...
![Request a Refund Confirmation Pop-up](/abel-bridge-v4/12-request-refund.png)
...
![Refund Completed Screen](/abel-bridge-v4/13-refund-completed.png)
...
![Recent Transactions List](/abel-bridge-v4/14-recent-transactions.png)

*Recent Transactions history section displaying past bridge activity.*

## Maintenance and Connection States

Version 4.0 adds guidance for temporary platform, bridge, and network interruption states. These situations do not change the normal bridge flow described in the previous sections.

### System Maintenance

When the platform is temporarily unavailable for system maintenance, the System Maintenance screen is displayed instead of the normal conversion interface.

- Users cannot create or process bridge conversions while the platform is unavailable.
- No action is required while maintenance is in progress.
- When the platform becomes available again, return to the conversion page and continue using the platform normally.

![System Maintenance Screen](/abel-bridge-v4/15-system-maintenance.png)

*System Maintenance screen displayed while the platform is temporarily unavailable.*

### Network Connection Issue

If the platform cannot confirm a network connection, the Network Connection Issue pop-up is displayed and the conversion interface is temporarily unavailable.

- While the platform is checking the connection, the button displays **Waiting...** and cannot be selected.
- When the platform connection becomes available again, the **Try again** button becomes enabled.
- Click **Try again** to reconnect and continue using the platform normally.

![Network Connection Issue - Waiting](/abel-bridge-v4/16-network-issue-waiting.png)

*Connection check in progress: Waiting... is disabled.*

![Network Connection Issue - Try Again](/abel-bridge-v4/17-network-issue-try-again.png)

*Connection restored: Try again is enabled.*

:::tip Note
If the connection issue continues after selecting **Try again**, wait until the network or platform connection is restored before retrying.
:::

### Bridge Under Maintenance

#### Creating a New Conversion Request

When only the bridge service is under maintenance, the conversion page remains accessible, but new bridge requests cannot be created.

- The bridge creation fields are disabled.
- Users cannot generate or submit a new conversion request during maintenance.
- Recent Transactions and existing request details remain available for viewing.
- When the bridge service becomes available again, the conversion fields return to normal and new requests can be created.

![Bridge Under Maintenance](/abel-bridge-v4/18-bridge-maintenance-new-request.png)

*Bridge Under Maintenance: new conversion fields are temporarily unavailable while transaction history remains accessible.*

#### Paused Conversion After Token Transfer

If the token transfer has already been completed and the bridge service enters maintenance, the existing conversion request is temporarily paused.

- The **Bridge Under Maintenance** status is displayed on the Bridge Detail screen.
- The request is not canceled.
- The **Retry** button remains disabled while the bridge is under maintenance.
- After the bridge service is restored, **Retry** becomes enabled.
- Click **Retry** to continue processing the existing conversion request.

![Paused Conversion - Retry Disabled](/abel-bridge-v4/19-paused-retry-disabled.png)

*During maintenance: Retry is disabled.*

![Paused Conversion - Retry Enabled](/abel-bridge-v4/20-paused-retry-enabled.png)

*After maintenance: Retry is enabled.*

:::tip Note
The conversion continues from the existing request after **Retry** is selected; a new conversion request is not required.
:::

#### Failed Conversion During Processing

If a conversion is already being processed when bridge maintenance interrupts processing, the request may change to **Bridge Failed**.

- The **Bridge Failed** status and reason are displayed in Bridge Detail.
- **Refund** is unavailable while the bridge service is still under maintenance.
- After the bridge service becomes available again, **Refund** becomes enabled.
- Click **Refund** and follow the existing refund flow to request the return of the transferred assets.

![Bridge Failed During Maintenance](/abel-bridge-v4/21-bridge-failed-maintenance.png)

*Bridge Failed status for a conversion affected during bridge maintenance.*

:::tip Note
This maintenance-related failure can be refunded after the bridge service is restored.
:::
